herbsters Core version *0.15.1* is now available from:

  <https://download.herbsters.org/herbsters-0.15.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/herbsters-dev>
...
installer (on Windows) or just copy over `/Applications/herbsters-Qt` (on Mac)
or `herbstersd`/`herbsters-qt` (on Linux).
...
herbsters Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

herbsters Core should also work on most other Unix-like systems but is not
...
A number of changes to the way herbsters Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 herbs per kB (0.00001 HERBS). This is to prepare for dropping the minimum transaction fee to 0.1 herbs per kB (0.0001 HERBS) in the next release.
...
- #11433 `e169349` Restore herbsters-util-test py2 compatibility (MarcoFalke)
- #11308 `2e1ac70` zapwallettxes: Wait up to 3s for mempool reload (MarcoFalke)
- #10798 `716066d` test herbsters-cli (jnewbery)
...
- #11125 `812c870` Add herbsters-cli -stdin and -stdinrpcpass functional tests (promag)
- #11077 `1d80d1e` fix timeout issues from TestNode (jnewbery)
- #11078 `f1ced0d` Make p2p-leaktests.py more robust (jnewbery)
- #11210 `f3f7891` Stop test_herbsters-qt touching ~/.herbsters (MeshCollider)
...
### Miscellaneous
- #11377 `75997c3` Disallow uncompressed pubkeys in herbsters-tx [multisig] output adds (TheBlueMatt)
