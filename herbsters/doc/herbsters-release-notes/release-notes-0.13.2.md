herbsters Core version 0.13.2 is now available from:

  <https://download.herbsters.org/herbsters-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/herbsters-project/herbsters/issues>
...
updates will be released anymore. Without security updates, using a herbsters
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of herbsters Core
randomly crashing on Windows XP. It is [not clear](https://github.com/herbsters/herbsters/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside herbsters transactions now use validation using
[libsecp256k1](https://github.com/herbsters-core/secp256k1) instead of OpenSSL.
...
(https://github.com/herbsters/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of herbsters Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of herbsters Core was to raise the default minimum
relay fee.

herbsters Core 0.13.2 will have a strict maximum size on the mempool. The
...
herbsters Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running herbstersd without having to do any manual configuration.
...
herbsters Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. herbsters Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In herbsters Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in herbsters Core 0.13, and it is at this time undecided whether the
...
herbsters Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
herbsters Core automatically creates a hidden service to listen on, without
manual configuration. herbsters Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if herbsters Core is listening, and
...
herbstersd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals herbsters
Core to use floating fees. By default, herbsters Core will use floating
fees.
...
(default: `0.0002` HERBS/kB).

At all times, herbsters Core will cap fees at `-maxtxfee=<x>` (default:
0.10) HERBS.
Furthermore, herbsters Core will never create transactions paying less than
...
`herbsters.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of herbsters Core v0.11.0](https://github.com/herbsters/herbsters/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/herbsters/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/herbsters/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /herbstersrpc>
        ...
        # optional bypass herbstersd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< herbstersrpc:<password>
    </Location>
...
Restarting herbstersd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in herbsters.conf
...
herbsters-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/herbsters-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The herbsters Core code base has
...
- `herbsters-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `herbsters-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/herbsters/herbsters/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the herbsters scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/herbsters/herbsters/pull/7524>
...
herbsters transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/herbsters/herbsters/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/herbsters/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of herbsters Core.

[Pull request](https://github.com/herbsters/herbsters/pull/8035/files), [BIP 32](https://github.com/herbsters/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/herbsters/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/herbsters/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the herbsters-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/herbsters/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/herbsters/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/herbsters/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/herbsters/herbsters/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming herbsters wallet
  software more difficult and which seriously complicates the design of smart
  contracts for herbsters.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each herbsters block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of herbsterss they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of herbsterss being spent is exactly
...
- **Increased security for multisig:** herbsters addresses (both P2PKH addresses
  ...
  believed to be provided by herbsters's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  herbsters paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of herbsters's current ledger (called the UTXO set),
  ...
  note that herbsters Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  herbsters users to individually opt-in to almost any change in the herbsters
  Script language when those users receive new transactions.  Features
  currently being researched by herbsters and herbsters Core contributors that may
  ...
  Other herbsters community members are studying several other improvements
...
segwit's start date of 1 January 2017 miners can update the herbsters
client to herbsters Core 0.13.2 to signal for segwit support. When a
...
[Segwit FAQ]: https://herbsterscore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://herbsterscore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/herbsters/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/herbsters/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/herbsters/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/herbsters/bips/blob/master/bip-0145.mediawiki
...
Since herbsters Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/herbsters/bips/blob/master/bip-0147.mediawiki
...
- `herbsters-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `herbsters-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- [The herbsters Core Developers](/doc/release-notes)
- Charles Lee
- Adrian Gallagher
- shaolinfry
- Xinxi Wang
